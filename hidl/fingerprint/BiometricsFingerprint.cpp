...
using OplusAuthType = vendor::oplus::hardware::biometrics::fingerprint::V2_1::FingerprintAuthType;
...
Return<uint64_t> BiometricsFingerprint::setNotify(
    ...
    // Required init for Egis sensor
    mOplusBiometricsFingerprint->cleanUp();
    ALOGE("setNotify cleanup done");
    mOplusBiometricsFingerprint->setTouchEventListener();
    ALOGE("setNotify touch listener set");
...
Return<RequestStatus> BiometricsFingerprint::authenticate(uint64_t operationId, uint32_t gid)  {
    ALOGE("auth");
    // Egis requires authenticateAsType with TYPE_KEYGUARD for lockscreen
    return OplusToAOSPRequestStatus(
        mOplusBiometricsFingerprint->authenticateAsType(operationId, gid, OplusAuthType::TYPE_KEYGUARD));
}
